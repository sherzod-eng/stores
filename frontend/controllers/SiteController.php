...
class SiteController extends Controller
{
    ...
    public function actionAddStore()
    {
        ...
        if($model->load(Yii::$app->request->post()) && $model->validate()){
            $model->save();
            echo Yii::$app->session->setFlash('success', 'Ok');
            $model = new Store();

        }
        
        $allStore = $model->getAll();
    ...
    public function actionInsertProduct()
    {
        ...
        if($model->load(Yii::$app->request->post())){
            $model->operation_date = $exp;
            $model->save();
            Yii::$app->session->setFlash('success', 'Ok');
            $model = new Operation();

        }
        return $this->render('insert-product', [
            'model' => $model
        ]);
    }
    ...
    public function actionAjax()
    {
        if(Yii::$app->request->isAjax){
            ...
            $model = new Operation();
            sleep(2);
            return $model->Result($id, $data);
        }
    }

   public function actionSorov()
   {
       if(Yii::$app->request->isAjax){
           $out = Yii::$app->request->get('out');
           $pro = Yii::$app->request->get('pro');
           $model = new Operation();
          return $model->getRes($out, $pro);
       }
   }
}
